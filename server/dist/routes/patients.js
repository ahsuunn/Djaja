...
router.get('/', auth_1.auth, (0, auth_1.authorize)('doctor', 'nakes', 'admin'), async (req, res) => {
    try {
...
router.post('/', auth_1.auth, (0, auth_1.authorize)('nakes', 'admin', 'doctor'), async (req, res) => {
    try {
        ...
        const patient = new Patient_1.default(patientData);
        await patient.save();
        // Create audit log
...
router.put('/:id', auth_1.auth, (0, auth_1.authorize)('nakes', 'admin', 'doctor'), async (req, res) => {
    try {
        // Transform legacy fields to FHIR format
        ...
        // Update meta version
        const existingPatient = await Patient_1.default.findById(req.params.id);
        ...
        const patient = await Patient_1.default.findByIdAndUpdate(req.params.id, updateData, { new: true });
        if (!patient) {
            res.status(404).json({ message: 'Patient not found' });
            return;
        }
...
// @access  Private
router.get('/:id/observations', auth_1.auth, async (req, res) => {
    try {
