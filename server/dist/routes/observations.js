...
router.get('/', auth_1.auth, async (req, res) => {
    try {
        ...
        const observations = await Observation_1.default.find(filter)
...
router.post('/', auth_1.auth, (0, auth_1.authorize)('nakes', 'doctor', 'admin'), async (req, res) => {
    try {
        ...
        const observation = new Observation_1.default(observationData);
...
router.put('/:id/review', auth_1.auth, (0, auth_1.authorize)('doctor'), async (req, res) => {
    try {
        const { doctorNotes } = req.body;
        const observation = await Observation_1.default.findByIdAndUpdate(req.params.id, {
            doctorNotes,
            reviewedBy: req.user._id,
            reviewedAt: Date.now(),
            updatedAt: Date.now(),
        }, { new: true }).populate('patientId').populate('performedBy').populate('reviewedBy');
        if (!observation) {
...
router.get('/stats/summary', auth_1.auth, async (req, res) => {
    try {
        const totalTests = await Observation_1.default.countDocuments();
        const criticalTests = await Observation_1.default.countDocuments({ overallStatus: 'critical' });
        const warningTests = await Observation_1.default.countDocuments({ overallStatus: 'warning' });
        const normalTests = await Observation_1.default.countDocuments({ overallStatus: 'normal' });
        const recentTests = await Observation_1.default.find()
            .sort({ createdAt: -1 })
            .limit(5)
            .populate('patientId', 'name patientId')
